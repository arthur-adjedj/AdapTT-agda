...
-- VarInstTelExtTel
  vinst++ : {Γ : Ctx} (Θ : Tel Γ) (A : Tel (Γ ▹₃[ + ] Θ)) → vinst {Γ} (Θ ++ₜ A) ≡ _extᵢ_ {(Γ ▹₃[ + ] Θ) ▹₃[ + ] A} { Θ [ WkTel + (Θ ++ₜ A) ]₃ } (vinst {Γ} Θ [ WkTel {Γ ▹₃[ + ] Θ} + A ]₄) {{!A [ ? ]₃!}} (vinst {Γ ▹₃[ + ] Θ} A) -- (vinst {Γ ▹₃[ + ] Θ} A)
...
▹▹₃++ₜₐ σ Θ₁' ta₁ Θ₂' ta₂ = {!refl !}
--   ▹ₛᵢ▹ₛᵢ : {Γ Δ : Ctx} {σ : Sub Γ Δ} {Θ : Tel Δ} (ι : Inst Γ (Θ [ σ ]₃)) {A : Tel (Δ ▹₃[ + ] Θ)} (ι' : Inst (Γ ▹₃[ + ] (Θ [ σ ]₃)) (A [ σ ▹▹₃[ + ]⟦ Θ , idₜₐ ⟧ ]₃))
...
--     → ta₁ ++ₜₐ⟦ Θ₂' ++ₜ Θ₃' , ta₂ ++ₜₐ⟦ {!  Θ₃ [ ? ]₃  !} , ta₃  ⟧  ⟧ ≡  (ta₁ ++ₜₐ⟦ Θ₂' , ta₂ ⟧) ++ₜₐ⟦ Θ₃' , ta₃ ⟧
